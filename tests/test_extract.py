...
from cropdusters.extract import extract_crop_chooser

import cropdusters.extract as extract
import datetime

class TestExtractCropChooser(unittest.TestCase):
    
    @patch('cropdusters.extract.requests.get')
...
from cropdusters.extract import extract_weather
...
class TestExtractWeather(unittest.TestCase):
    ...
    def test_extract_weather(self, mock_get):
        ...
        actual_data = extract_weather(latitude, longitude, start_date, end_date) #Excluding variables, timezone
        print(actual_data)
        
        self.assertEqual(actual_data, expected_data)

from cropdusters.extract import extract_crop_yields
